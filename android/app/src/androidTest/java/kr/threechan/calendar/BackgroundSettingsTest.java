...
public class BackgroundSettingsTest {
 ...
 private void waitFor(ActivityScenario<MainActivity> app,String expression)throws Exception{
  for(int i=0;i<100;i++){if("true".equals(js(app,expression)))return;SystemClock.sleep(100);}fail(expression);
 }
 ...
 @Test public void onlyUnmetSettingsHaveActionsAndReturnRefreshesThem()throws Exception{
  ...
  try(ActivityScenario<MainActivity> app=ActivityScenario.launch(MainActivity.class)){
   ...
   js(app,"(()=>{const buttons=[...document.querySelectorAll('button')];const settings=buttons.find(b=>b.getAttribute('aria-label')==='설정')||buttons.find(b=>b.getAttribute('aria-label')==='동기화 설정 열기')||buttons.find(b=>b.textContent.trim().startsWith('설정'));settings?.click();})()");
