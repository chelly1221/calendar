    ...
    final class NativeBridge {
        @JavascriptInterface public String invoke(String method,String input){
            try{
                ...
                switch(method){
                    case "device:status":result=new DeviceCalendars(getApplicationContext()).sources();break;
                    case "device:scan":result=new DeviceCalendars(getApplicationContext()).scan();break;
                    case "device:write":result=new DeviceCalendars(getApplicationContext()).write(args);break;
                    case "device:remove":result=new DeviceCalendars(getApplicationContext()).remove(args);break;
                    case "device:completed":DeviceCalendars calendars=new DeviceCalendars(getApplicationContext());calendars.completed();result=calendars.sources();break;
                    case "finish":
                    ...
                    case "publishWidget": kr.threechan.calendar.widget.WidgetStore.write(getApplicationContext(),args.getJSONObject("snapshot"));kr.threechan.calendar.widget.CalendarWidgetProvider.updateAll(getApplicationContext());result=new JSONObject();break;
