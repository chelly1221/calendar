...
public class MainActivity extends BridgeActivity {
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        getSharedPreferences("calendar_credentials", MODE_PRIVATE).edit().clear().apply();
        deleteSharedPreferences("calendar_credentials");
        try {
            KeyStore keys = KeyStore.getInstance("AndroidKeyStore");
            keys.load(null);
            if (keys.containsAlias("calendar.session.key.v1")) keys.deleteEntry("calendar.session.key.v1");
    ...
    void closeAuthTab() {
        ...
        Intent returnToCalendars = new Intent(this, MainActivity.class);
        returnToCalendars.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
        startActivity(returnToCalendars);
        authTabOpen = false;
        authCloseRequested = false;
    }
    ...
    @Override public void onSaveInstanceState(Bundle outState) {
        outState.putBoolean("calendar.authTabOpen", authTabOpen);
