// Copyright (c) 2026 Calendar contributors
...
const calendarOrigin = "https://audax-vm.tail62313c.ts.net:8445"
const maxCalendarBytes = 16 << 20

var requestMu sync.Mutex
var requestCancels = map[string]context.CancelFunc{}

func calendarClient(i *jsIPN) *http.Client {
...
func calendarContext(id string, duration time.Duration) (context.Context, func()) {
...
func calendarExports(i *jsIPN, exports map[string]any) map[string]any {
	client := calendarClient(i)
	...
	exports["request"] = js.FuncOf(func(_ js.Value, args []js.Value) any {
		...
		return makePromise(func() (any, error) {
			...
			if !strings.HasPrefix(pathname, "/api/") || strings.ContainsAny(pathname, "\\\r\n#") ||
				...
				return nil, fmt.Errorf("unsupported calendar request")
			}
			bodyJS := options.Get("body")
			if bodyJS.Length() > maxCalendarBytes {
				return nil, fmt.Errorf("request too large")
			}
			body := make([]byte, bodyJS.Length())
			js.CopyBytesToGo(body, bodyJS)
			ctx, done := calendarContext(options.Get("id").String(), 45*time.Second)
			defer done()
			req, err := http.NewRequestWithContext(ctx, method, calendarOrigin+pathname, bytes.NewReader(body))
			if err != nil {
				return nil, err
			}
			headers := options.Get("headers")
			for _, name := range []string{"content-type", "x-calendar-request", "x-filename"} {
			...
			data, err := io.ReadAll(io.LimitReader(res.Body, maxCalendarBytes+1))
			if err != nil {
				return nil, err
			}
	...
	exports["watch"] = js.FuncOf(func(_ js.Value, args []js.Value) any {
		...
		return makePromise(func() (any, error) {
			ctx, done := calendarContext(id, 5*time.Minute)
			defer done()
			req, _ := http.NewRequestWithContext(ctx, "GET", calendarOrigin+"/api/sync/events", nil)
			res, err := client.Do(req)
			if err != nil {
				return nil, err
			}
	...
	// This application exposes only its fixed calendars API, never SSH or arbitrary TCP/HTTP.
