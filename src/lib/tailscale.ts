...
function setup() {
  ...
  if (typeof SharedWorker !== "undefined") {
    try {
      owner = new SharedWorker(url, { name: "calendar-tailscale-0.1.0" });
      port = owner.port;
      port.start();
    } catch {
  ...
  owner.onerror = () => {
    const message = "보안 연결을 열지 못했어요. 달력을 다시 열어 주세요.";
...
export async function ensureTailscale(): Promise<void> {
  ...
  if (existing && (typeof window === "undefined" || !window.BackgroundSyncNative) && (snapshot.state === "NeedsLogin" || snapshot.state === "Stopped"))
...
export async function tailscaleFetch(input: string, options: RequestInit = {}): Promise<Response> {
  // Every calendar request crosses the embedded WireGuard stack. No native fetch fallback.
  const url = new URL(input);
  if (url.origin !== "https://audax-vm.tail62313c.ts.net:8445" || !url.pathname.startsWith("/api/"))
    throw new Error("허용되지 않은 달력 서버 주소예요.");
