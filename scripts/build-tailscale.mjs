...
bridge = bridge.replace(
  ...
  '_ = c // Calendar does not send diagnostic logs to a third party.\n\tlogf := logger.Discard',
);
bridge = bridge.replace(
...
bridge = bridge.replace(
  ...
  'return calendarExports(jsIPN, map[string]any{\n\t\t"run":',
);
bridge = bridge.replace(
  '\n\t}\n}\n\ntype jsIPN struct',
  '\n\t})\n}\n\ntype jsIPN struct',
...
for (const name of ['calendar_js.go', 'isrgrootx1.crt', 'isrgrootx2.crt'])
  await fs.copyFile(
    path.join(root, 'networking/tailscale', name),
    path.join(source, 'cmd/tsconnect/wasm', name),
  );
await fs.mkdir(path.join(source, 'cmd/calendar-build-flags'), { recursive: true });
await fs.writeFile(
  path.join(source, 'cmd/calendar-build-flags/main.go'),
...
const tags = run(go, ['run', './cmd/calendar-build-flags'], {
  encoding: 'utf8',
...
let notices =
  'Third-party software included in the Calendar embedded Tailscale build.\n';
